...
class SecurityProvider with ChangeNotifier {
    ...
    try {
      ...
      _apkSubscription = _apkCentinel.onApkInstalled.listen(
        ...
        onError: (Object error, StackTrace stackTrace) {
          _appendLog('ERROR STREAM APK CENTINEL: $error');
          debugPrint('[JOSH APK] Stream error: $error\n$stackTrace');
        },
      );
      ...
      _initialized = true;
      _appendLog('CENTINELA INICIALIZADO CORRECTAMENTE.');
    } catch (e, stackTrace) {
      _initialized = false;
      _statusCategory = 'ERROR DE INICIALIZACIÓN';
      _appendLog('ERROR INICIALIZANDO CENTINELA: $e');
      debugPrint('[JOSH SECURITY] Error inicializando: $e\n$stackTrace');
    } finally {
  ...
  Future<void> _handleApkInstallEvent(ApkInstallEvent event) async {
    try {
      final String appName = event.appName.trim().isNotEmpty
          ? event.appName.trim()
      ...
      if (isOfficialOrSystem) {
        _appendLog('APK OFICIAL/SISTEMA DETECTADO: $appName', notify: false);
      } else {
        _appendLog('APK NO VERIFICADO DETECTADO: $appName', notify: false);
      }
      ...
      _appendLog('ERROR PROCESANDO EVENTO APK: $e');
      debugPrint('[JOSH APK] Error procesando instalación: $e\n$stackTrace');
    }
  }

  bool _isOfficialOrSystemApkPath(String path) {
  ...
  Future<bool> pickLocalFile() async {
    try {
      ...
      debugPrint('[JOSH FILE] Error seleccionando archivo: $e\n$stackTrace');
      return false;
    }
  }
  ...
  Future<void> executeAuditoria(
    ...
    if (cleanTarget.isEmpty && vector != 2) {
      _appendLog('AUDITORÍA CANCELADA: objetivo vacío.');
      return;
    }

    if (_isEnginePatrolling) {
      _appendLog('AUDITORÍA IGNORADA: Centinela ya está procesando.');
      return;
    }
    ...
    try {
      switch (vector) {
        case 0:
          await _processCallScan(cleanTarget, showOverlay: showCallOverlay);
          break;
        case 1:
          await _processUrlScan(cleanTarget);
          break;
        case 2:
          await _auditFile();
          break;
        default:
          _appendLog('VECTOR DE SEGURIDAD NO SOPORTADO: $vector');
          break;
      }
      await _loadHistoricalLogs();
    } catch (e, stackTrace) {
      _appendLog('ERROR GENERAL DE AUDITORÍA: $e');
      debugPrint('[JOSH ENGINE] Error general: $e\n$stackTrace');
    } finally {
  ...
  Future<void> processIncomingCall(String phoneNumber) async {
    ...
    _appendLog('LLAMADA ENTRANTE DETECTADA: $number', notify: false);
    _statusCategory = 'ANALIZANDO LLAMADA ENTRANTE';
    notifyListeners();

    try {
      await executeAuditoria(number, 0, showCallOverlay: true);
    } catch (e, stackTrace) {
      ...
      if (showOverlay) {
        try {
          await OverlayService.showWarningOverlay(
            phoneNumber: number,
            riskLevel: verdict,
            message: buildOverlayMessage(score, verdict),
            agentReasoning: reasoning,
          );
        } catch (overlayError, overlayStack) {
          _appendLog('ERROR MOSTRANDO OVERLAY: $overlayError');
  ...
  double _mapFileVerdictToScore(FileScanVerdict verdict) {
    final double riskScore = verdict.riskScore.toDouble();
    if (verdict.isCritical) return riskScore.clamp(80.0, 100.0).toDouble();
    if (verdict.isWarning) return riskScore.clamp(40.0, 79.0).toDouble();
    return riskScore.clamp(0.0, 39.0).toDouble();
  }

  String _serializeFileVerdict(FileScanVerdict verdict) {
    return <String, dynamic>{
      'fileName': verdict.fileName,
      'fileSizeInBytes': verdict.fileSizeInBytes,
      'fileSizeInMB': verdict.fileSizeInMB,
      'riskScore': verdict.riskScore,
      'riskLevel': verdict.riskLevel,
      'analysisMessage': verdict.analysisMessage,
      'source': verdict.source.toString(),
      'telemetryDetails': verdict.telemetryDetails,
    }.toString();
  }
  ...
  Future<void> _persistAudit(
    ...
    try {
      ...
      _appendLog('ERROR GUARDANDO SCAN LOG: $e', notify: false);
      debugPrint('[JOSH DATABASE] insertScanLog error: $e\n$stackTrace');
    }
    ...
    try {
      ...
      _appendLog('ERROR GUARDANDO FORENSIC LOG: $e', notify: false);
      debugPrint('[JOSH DATABASE] insertForensicLog error: $e\n$stackTrace');
    }
  }
  ...
  void _appendLog(String text, {bool notify = true}) {
  ...
  Future<void> _loadHistoricalLogs() async {
    try {
      ...
      debugPrint('[JOSH DATABASE] Error cargando historial: $e\n$stackTrace');
    }
  }
